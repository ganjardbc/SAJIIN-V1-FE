          ...
          >
            <div style="width: 60px; margin-right: 20px">
              <div class="image image-padding image-circle">
                <img
                  :src="customerImageThumbnailUrl + dt.customer.image"
                  alt=""
                  class="post-center"
                />
              </div>
            ...
            <div
              class="display-flex column space-between"
            ...
            >
              <div class="display-flex space-between">
                ...
                <div
                  :class="
                    'card-capsule ' +
                    (dt.customer.status === 'active' ? 'active' : '')
                  "
              ...
              <div class="display-flex space-between">
                ...
                <div v-if="data" class="display-flex">
                  <button
                    ...
                    @click="onShowEdit(dt.customer.id)"
                  >
                    <i class="fa fa-1x fa-pencil-alt"></i>
                  </button>
                  <button
                    v-if="data.id !== dt.customer.id"
                    class="btn btn-small-icon btn-sekunder"
                    @click="onShowHideChoose(dt.customer)"
                  >
                    <i class="fa fa-1x fa-plus"></i>
                  </button>
                  <router-link
                    v-else
                    :to="{ name: 'customer-main' }"
...
export default {
  ...
  methods: {
    ...
    async getDataByID(id) {
      ...
      const rest = await axios.post('/api/customer/getByID', payload, {
        headers: { Authorization: token },
      })
    ...
    async getData() {
      ...
      const rest = await axios.post('/api/customer/getAll', payload, {
        headers: { Authorization: token },
      })
