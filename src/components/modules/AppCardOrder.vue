  ...
  <div id="AppCardOrder">
    ...
    >
      <div class="display-flex space-between" style="margin-bottom: 20px">
        <div class="display-flex">
          <div style="margin-right: 10px">
            <i class="fa fa-1x fa-shopping-bag fonts main-color"></i>
          </div>
          <div class="display-flex mobile-hidden">
            <div class="fonts fonts-10 grey">
              {{ dt.order.created_at | moment('dddd, MMMM Do YYYY hh:mm') }}
            </div>
        ...
        <div v-if="type === 'owner'" class="display-flex">
          <AppCapsuleMenu
            ...
            :onChange="
              (data) => onChangeStatus(data, dt.order.order_id, 'payment')
            "
          ...
          <AppCapsuleMenu
            ...
            "
            :onChange="
              (data) => onChangeStatus(data, dt.order.order_id, 'status')
            "
            :data="bizparCapsule"
        ...
        <div v-else class="display-flex">
          <div class="card-capsule" style="margin-right: 5px">
            {{ dt.order.payment_status ? 'Paid' : 'Unpaid' }}
          </div>
          <div
            :class="
              'card-capsule ' +
      ...
      <div v-if="dt.order.note">
        <div class="fonts fonts-10 grey"># {{ dt.order.note }}</div>
      </div>
      <div
        class="display-flex display-mobile"
        style="padding-top: 5px; padding-bottom: 5px"
      >
        <div
          class="width width-76 width-mobile border-right border-mobile-none no-margin-padding"
          style="padding-right: 10px; padding-bottom: 10px"
        >
          <AppImageProduct :data.sync="dt.details" :to="1" />
          <AppShowHide
            v-if="dt.details.length > 1"
            :title="'More Products (' + (dt.details.length - 1) + ')'"
          >
            <AppImageProduct
              :data.sync="dt.details"
              :from="1"
              :to="dt.details.length"
            />
          </AppShowHide>
        </div>
        <div class="width width-24 width-mobile">
          <div
            style="height: auto; padding-left: 20px"
            class="no-margin-padding"
          >
            <div style="padding-top: 10px; padding-bottom: 10px">
              <div v-if="dt.table">
                <div class="fonts fonts-9 grey" style="margin-bottom: 2px">
                  Table
                </div>
                <div
