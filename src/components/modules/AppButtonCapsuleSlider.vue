<template>
    <div id="AppButtonCapsuleSlider" class="slider display-flex space-between align-center wrap">
        <div class="horizontal-scroll" style="overflow-x: auto;">
                    ...
                    <div class="display-flex nowrap align-center" style="padding: 5px 10px 5px 5px;">
                        <div class="width width-35px">
                            <div class="image image-30px image-circle bg-transparent">
                                <i :class="`post-middle-absolute fonts fonts-10 grey ${customIcon ? customIcon : 'fa fa-lw fa-check'}`"></i>
                            </div>
                        </div>
            ...
            <div v-for="(dt, i) in filteredData" :key="i" style="position:relative; margin: 10px 13px 5px 0; flex-grow: 1; display: inline-block;">
                <div 
                    :class="`
                        card 
                        card-status 
                    ...
                    <div class="display-flex">
                        <div class="display-flex nowrap align-center" style="padding: 5px 10px 5px 5px;">
                            <div class="width width-35px">
                                <div class="image image-30px image-circle bg-transparent">
                                    <img v-if="dt.image" :src="dt.image" alt="" class="post-center">
                                    <i v-else :class="`post-middle-absolute fonts fonts-10 ${dt.status === 'active' ? 'black' : 'grey'} ${dt.icon ? dt.icon : 'fa fa-lw fa-check'}`"></i>
                                </div>
                            </div>
                            <div style="width: calc(100% - 35px);" class="display-flex align-center">
                                <div :class="`fonts fonts-10 semibold ${dt.status === 'active' ? 'black' : 'grey'}`">{{ dt.label }}</div>
                ...
                <div v-if="enableEditButton" class="slider-edit display-flex column center align-center">
                    <button class="btn btn-small-icon btn-circle btn-sekunder" @click="onEdit(dt)">
                        <i class="icn fonts fonts-8 fa fa-lw fa-edit"></i>
                    </button>
                </div>
            </div>
            <div v-if="enableCreateButton" style="width: 25px; flex-grow: 1; display: inline-block;"></div>
        </div>
        <div v-if="enableCreateButton" class="slider-create display-flex column center align-center">
            <button class="btn btn-small-icon btn-circle btn-sekunder" @click="onCreate">
