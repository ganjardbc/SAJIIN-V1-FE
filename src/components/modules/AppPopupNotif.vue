  ...
  <div id="App" style="position: relative">
    <div style="padding-bottom: 5px">
      <div
        ...
        :class="
          'card small-padding ' +
          (dt.is_read === 0 ? 'border-full bg-white-grey' : 'border-full')
        "
        ...
        >
          <div style="width: 40px; margin-right: 15px">
            <div class="image image-padding border border-full bg-white">
              <img
                v-if="dt.image"
                :src="notificationImageThumbnailUrl + dt.image"
                alt=""
                class="post-center"
              />
          ...
          <div style="width: calc(100% - 55px)">
            <div>
              <div class="fonts fonts-11 semibold" style="margin-bottom: 3px">
                {{ dt.title }}
              </div>
...
export default {
  ...
  data() {
    return {
      ...
      datas: [],
      bizparCapsule: [{ label: 'Read' }, { label: 'Unread' }],
      dataUser: null,
      limit: 10,
      offset: 0,
    }
  ...
  methods: {
    ...
    async getData(limit, offset) {
      ...
      const rest = await axios.post('/api/notification/getAll', payload, {
        headers: { Authorization: token },
      })
