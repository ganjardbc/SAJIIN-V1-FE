<template>
  <div id="App">
    <button
      v-if="!enableDetail"
      :class="'btn btn-main ' + (isFull ? 'btn-full' : '') + ' box-shadow'"
    ...
    >
      {{
        selectedTable && selectedTable.name
          ? selectedTable.name
          : 'Choose Visible Table'
      }}
      <i :class="'icn fa fa-lg fa-plus-circle'" />
    </button>

    <div
      v-else
      class="card box-shadow"
      style="margin-bottom: 15px; padding: 10px; width: calc(100% - 20px)"
    >
      <div v-if="selectedTable" class="display-flex space-between">
        <div style="width: 45px; margin-right: 15px">
          <div
            class="image image-padding border border-full"
            style="background-color: rgba(0, 0, 0, 0)"
          >
            <i
              v-if="!selectedTable.image"
            ...
            <img
              ...
              :src="
                selectedTable
                  ? tableImageThumbnailUrl + selectedTable.image
                  : ''
              "
            />
          </div>
        ...
        <div style="width: calc(100% - 100px)">
          <div class="post-top">
            ...
            <div class="fonts fonts-10 grey">
              {{ selectedTable && selectedTable.description }}
            </div>
          </div>
        </div>
