                ...
                <div class="width width-100 padding padding-top-5px">
                    ...
                    <div class="width width-100 display-flex align-center margin margin-bottom-15px">
                        <div style="width: calc(50% - 20px);">
                            ...
                            <div class="fonts fonts-11 semibold main-color overflow-ellipsis">{{ format(form.cash_summary) }}</div>
                        </div>
                        ...
                        <div style="width: calc(50% - 20px);">
                            ...
                            <div class="fonts fonts-11 semibold black overflow-ellipsis align-right">{{ format(form.cash_actual) }}</div>
                        </div>
                    </div>
                        ...
                        <div class="card bg-white-grey no-padding display-flex space-between align-center wrap">
                            <div class="width width-50 padding padding-top-10px padding-bottom-10px">
                                <div class="padding padding-5px content-center">
                                    ...
                                    <div class="fonts fonts-10 semibold black overflow-ellipsis align-center">{{ format(form.cash_modal) }}</div>
                                </div>
                            </div>
                            <div class="width width-50 padding padding-top-10px padding-bottom-10px">
                                <div class="padding padding-5px border-left content-center">
                                    ...
                                    <div class="fonts fonts-10 semibold black overflow-ellipsis align-center">{{ format(form.cash_profit) }}</div>
                                </div>
                            </div>
                            <div class="width width-50 padding padding-top-10px padding-bottom-10px">
                                <div class="padding padding-5px content-center">
                ...
                <div v-loading="loadingReport" class="width width-100">
                    <div v-if="selectedIndex === 0" class="width width-100">
                        ...
                        <div 
                            ...
                            <div v-if="dt.order.cashier_name" class="display-flex space-between align-center">
                                <div class="fonts fonts-10 normal black">Kasir</div>
                                <div class="fonts fonts-10 semibold black align-right">{{ dt.order.cashier_name }}</div>
                            </div>
                            ...
                            <div v-if="dt.order.table_id" class="display-flex space-between align-center">
                            ...
                            <div v-if="dt.order.platform_id" class="display-flex space-between align-center">
                            ...
                            <div class="display-flex space-between">
                                <div class="display-flex">
                                    ...
                                    <div class="fonts fonts-10 semibold black align-right margin margin-left-5px">({{ dt.order.payment_status ? 'Dibayar' : 'Belum Bayar' }})</div>
                                </div>
                                <div class="fonts fonts-10 semibold black align-right">{{ format(dt.order.total_price) }}</div>
                            </div>
                            <AppCardCollapse :title="`${dt.order.total_item} Produk`" class="padding padding-top-15px">
                                <el-table :data="dt.details" border empty-text="No Data" class="margin margin-top-15px margin-bottom-15px">
                                    <el-table-column label="Produk" min-width="160">
                                        <template slot-scope="scope">
                                            <div class="fonts fonts-9 normal black">{{ scope.row.product_name }} {{ scope.row.product_detail ? `- ${scope.row.product_detail}` : '' }}</div>
                                            <div class="fonts fonts-9 semibold black">{{ format(scope.row.price) }}</div>
                                        </template>
                                    </el-table-column>
                                    <el-table-column label="Qty" align="center" width="50">
                                        <template slot-scope="scope">
                                            <div class="fonts fonts-9 semibold black">{{ scope.row.quantity }}</div>
                                        </template>
                                    </el-table-column>
                    ...
                    <div v-if="selectedIndex === 1" class="width width-100">
                        ...
                        <div 
                            v-for="(dt, i) in dataExpense" :key="i" 
                            class="card bg-white box-shadow margin margin-bottom-15px">
                            <div class="display-flex space-between align-center margin margin-bottom-8px">
                                <div class="fonts fonts-10 semibold black">{{ dt.expense.expense_list_id }}</div>
                                <AppCardCapsule :data="dt.expense.status" />
                            </div>
                            <div class="display-flex space-between align-center">
                                <div class="fonts fonts-10 normal black">Tanggal</div>
                                <div class="fonts fonts-10 semibold black align-right">{{ dt.expense.expense_date | moment("DD MMM YYYY") }}</div>
                            </div>
                            <div class="display-flex space-between align-center">
                                <div class="fonts fonts-10 normal black">Biaya</div>
                                <div class="fonts fonts-10 semibold black align-right">{{ format(dt.expense.expense_price) }}</div>
                            </div>
                            ...
                            <div v-if="dt.expense.description" class="display-flex space-between align-center">
                                ...
                                <div class="fonts fonts-10 semibold black align-right">{{ dt.expense.description }}</div>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div v-if="selectedIndex === 0" class="margin margin-bottom-15px">
                    <div class="display-flex space-between">
                        ...
                        <div class="display-flex column align-right">
                            ...
                            <div v-if="grandItem" class="fonts fonts-9 normal black">{{ grandItem }} item</div>
                        </div>
                    </div>
                </div>
