    ...
    methods: {
        ...
        async onPrintToThermal () {
            try {
                ...
                // order 
                await printer.writeValue(encoder.encode(alignLeft + 'ID Pesanan : ' + data.order_id +  CMD.EOL))
                await printer.writeValue(encoder.encode('Tanggal    : ' + m(data.created_at).format('DD/MM/YYYY') +  CMD.EOL))
                if (data.customer_name) {
                    await printer.writeValue(encoder.encode('Pelanggan  : ' + replaceString(data.customer_name) + CMD.EOL))
                }
                if (data.cashier_name) {
                    await printer.writeValue(encoder.encode('Kasir      : ' + replaceString(data.cashier_name) + CMD.EOL))
                }
                if (data.table_name) {
                    await printer.writeValue(encoder.encode('Meja       : ' + replaceString(data.table_name) + CMD.EOL))
                }
                if (data.platform_name) {
                    await printer.writeValue(encoder.encode('Platform   : ' + replaceString(data.platform_name) + CMD.EOL))
                }
                await this.sendPrintLine(printer)
                ...
                if (data.details && data.details.length > 0) {
                    ...
                    for (let i = 0; i < data.details.length; i++) {
                        ...
                        await printer.writeValue(encoder.encode(alignRight + element.quantity + ' x ' + formatCurrency(element.price) +  CMD.EOL))
                    }
                ...
                await printer.writeValue(encoder.encode(alignLeft + 'Kembali    : ' + formatCurrency(data.change_price) + CMD.EOL))

                await printer.writeValue(encoder.encode(CMD.EOL + alignCenter + 'THANK YOU FOR YOUR ORDER !'))
                await printer.writeValue(encoder.encode(CMD.EOL + alignCenter + fontSmall + 'Powered by Saji-In' + CMD.EOL))
