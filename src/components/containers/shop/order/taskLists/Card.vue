                            ...
                            <div class="card bg-white border border-full">
                                <div class="display-flex align-center space-between margin margin-bottom-10px">
                                    ...
                                    <div style="width: calc(100% - 45px);">
                                        <div class="fonts fonts-11 semibold overflow-ellipsis">{{ detail.product_name }}</div>
                                        <div class="fonts fonts-9 normal grey overflow-ellipsis">{{ detail.quantity }} x {{ detail.product_detail }}</div>
                                        <div v-if="detail.note" class="fonts fonts-9 normal grey overflow-ellipsis">{{ detail.note }}</div>
                                    </div>
                                </div>
                                <div class="width width-100">
                                    <div v-if="detail.employee" class="padding padding-10px border-small-radius bg-white-grey margin margin-bottom-10px">
                                        <div class="display-flex space-between">
                                            ...
                                            <div style="width: calc(100% - 50px);">
                                                <div class="fonts fonts-11 semibold overflow-ellipsis">{{ detail.employee.name }}</div>
                                                <div class="fonts fonts-9 grey overflow-ellipsis">{{ detail.updated_at | moment("DD/MM/YYYY hh:mm") }}</div>
                                            </div>
                                        </div>
                                    </div>
                                    <button 
