...
const getCashbookList = (cashBook) => {
  let data = [{ value: '', label: 'Semua Buku Kas' }]
  cashBook &&
    cashBook.map((item) => {
      data.push({
        value: item.id,
...
const getDiscountOrder = (order, calculation) => {
  ...
  if (calculation.current_status === 'create') {
    let discountPriceCalculation =
      calculation.discount_value_type === 'percentage'
...
const getDiscountProduct = (data, calculation) => {
  ...
  if (calculation.current_calculation === 'discount') {
    if (calculation.current_status === 'create') {
      ...
      if (isPlatform) {
        platformPrice =
          data.platform_currency_type === 'percentage'
  ...
  if (calculation.current_calculation === 'platform') {
    if (calculation.current_status === 'create') {
      ...
      platformPrice =
        calculation.platform_currency_type === 'percentage'
      ...
      if (isDiscount) {
        discountPrice =
          data.discount_value_type === 'percentage'
